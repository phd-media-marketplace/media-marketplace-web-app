...
import type { MediaPartnerFormData } from "../../types";
...
export default function MediaPartnerRegistrationForm() {
    const navigate = useNavigate();
    const [showPassword, setShowPassword] = useState(false);
    const [countryCode, setCountryCode] = useState('+233');
    const { control, handleSubmit, formState: { errors } } = useForm<MediaPartnerFormData>({
        defaultValues: {
            email: '',
            password: '',
            firstName: '',
            ...
            tenantSlug: '',
            phoneNumber: '',
            tenantType: 'MEDIA_PARTNER', // fixed tenant type for media partner registration
            mediaType: 'TV', // default media type
        }
    });
    ...
    const onSubmit = (data: MediaPartnerFormData) => {
    ...
    >
        <div className="relative w-full max-w-120 mx-auto">
            ...
            >
                ...
                <p className="text-black/50 mb-6">Sign up to start receiving client requests and manage your media campaigns.</p>
            </motion.div>
            ...
            >
                <div>
                    <Controller
                        ...
                        control={control}
                        rules={{
                            required: {value: true, message: 'Tenant/Organization name is required'},
                            minLength: {value: 1, message: 'Tenant/Organization name must be at least 1 character'},
                            maxLength: {value: 100, message: 'Tenant/Organization name must be at most 100 characters'},
                        }}
                        render={({ field }) => (
                        ...
                        render={({ field }) => (
                            <div className="flex items-center gap-3 px-4 py-3 border border-gray-300 rounded-lg focus-within:ring-2 focus-within:ring-[#C8F526]">
                                ...
                                <Select onValueChange={(value) => field.onChange(value)}>
                                    <SelectTrigger className="w-full focus:outline-none bg-transparent border-none">
                                        <SelectValue placeholder="Select media type" className="focus:outline-none text-gray-700" />
                                    </SelectTrigger>
                                    <SelectContent className="bg-white border-none text-accent focus:bg-transparent">
