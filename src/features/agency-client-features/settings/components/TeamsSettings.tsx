...
const INITIAL_MEMBERS: TeamMember[] = [
  { firstName: "Grace", lastName: "Ofori", role: "Admin", email: "grace@agency.com", status: "ACTIVE" },
  { firstName: "Kofi", lastName: "Nkrumah", role: "Sales Associate", email: "kofi@agency.com", status: "ACTIVE" },
  { firstName: "Yaw", lastName: "Asare", role: "Finance", email: "yaw@agency.com", status: "INACTIVE" },
];
...
export default function TeamsSettings() {
  ...
  const roleLabelByValue = useMemo(() => INVITE_ROLE_OPTIONS.reduce<Record<string, string>>((acc, option) => {
    acc[option.value] = option.label;
    return acc;
  }, {}), []);

  const roleValueByLabel = useMemo(() => INVITE_ROLE_OPTIONS.reduce<Record<string, string>>((acc, option) => {
  ...
  const toggleMemberStatus = (email: string) => {
    setMembers((prev) => prev.map((member) => {
      ...
      if (member.status === "PENDING") return { ...member, status: "ACTIVE" };
  ...
  const handleEditMember = (email: string, role: string) => {
    ...
    const normalizedEmail = email.trim().toLowerCase();
    const duplicate = members.some((member) => member.email.toLowerCase() === normalizedEmail && member.email.toLowerCase() !== editingMember.email.toLowerCase());
    if (duplicate) {
      toast.error("Another team member already uses this email.");
      return;
    }

    setMembers((prev) => prev.map((member) => member.email === editingMember.email ? { ...member, email: normalizedEmail, role: roleLabelByValue[role] ?? role } : member));
  ...
  const teamTableColumns = useMemo<UniversalDataTableColumn<TeamMember>[]>(() => [
    {
      id: "name",
      header: "Name",
      cell: (member) => <span className="font-medium text-slate-900">{`${member.firstName} ${member.lastName}`}</span>,
      widthPx: 280,
      minWidthPx: 220,
      widthClassName: "w-70",
  ...
  const headerSlot = (
    <div className="flex flex-col gap-3 border-b border-slate-200 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-2">
        ...
        <Badge variant="outline" className="text-xs">{filteredMembers.length}</Badge>
      </div>

      <div className="relative w-full sm:max-w-xs">
        <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
        <Input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search by name, email, role..." className="filter-input-field" />
      </div>
    </div>
  );
    ...
    <div className="space-y-6 pb-10">
      ...
      <div>
        <DataTable
          ...
          emptyState={<NoDataCard title="No team members yet" message="Invite team members to collaborate on your agency account." btnText="Invite Team Member" redirectFunc={() => setOpenInviteDialog(true)} className="mx-auto max-w-xl" />}
        />
      </div>
    </div>
  );
